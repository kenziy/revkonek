...
import { ClubSubscriptionCoupon, CouponDiscountType } from '@/types/club';

interface Props {
    coupons: PaginatedData<ClubSubscriptionCoupon>;
    filters: {
        search?: string;
    };
}
...
export default function ClubSubscriptionCoupons({ coupons, filters }: Props) {
    const [search, setSearch] = useState(filters.search || '');
    const [showModal, setShowModal] = useState(false);
    const [editingCoupon, setEditingCoupon] = useState<ClubSubscriptionCoupon | null>(null);

    const { data, setData, post, put, processing, errors, reset, clearErrors } = useForm<CouponFormData>(defaultForm);

    const handleSearch = (value: string) => {
        router.get(route('admin.club-subscriptions.coupons'), { search: value }, { preserveState: true });
    };
    ...
    const openEdit = (coupon: ClubSubscriptionCoupon) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingCoupon) {
            put(route('admin.club-subscriptions.coupons.update', editingCoupon.id), {
                onSuccess: () => setShowModal(false),
            });
        } else {
            post(route('admin.club-subscriptions.coupons.store'), {
                onSuccess: () => setShowModal(false),
            });
        }
    };

    const handleToggle = (coupon: ClubSubscriptionCoupon) => {
        router.post(route('admin.club-subscriptions.coupons.toggle', coupon.id));
    };

    const handleDelete = (coupon: ClubSubscriptionCoupon) => {
        if (confirm(`Delete coupon "${coupon.code}"?`)) {
            router.delete(route('admin.club-subscriptions.coupons.destroy', coupon.id));
        }
    };
