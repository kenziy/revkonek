...
export default function ClubSubscriptionShow({ subscription }: Props) {
    ...
    const handleVerify = () => {
        ...
        setProcessing(true);
        router.post(
            route('admin.club-subscriptions.verify', subscription.id),
            {
                action,
        ...
        <AdminLayout header="Subscription Detail">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <Card>
                        ...
                        <dl className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                <dt className="text-secondary-500 dark:text-secondary-400">Club</dt>
                                <dd className="font-medium text-secondary-900 dark:text-white mt-1">
                                    {subscription.club?.name || 'N/A'}
                                </dd>
                            </div>
                            <div>
                                <dt className="text-secondary-500 dark:text-secondary-400">Requested By</dt>
                                <dd className="font-medium text-secondary-900 dark:text-white mt-1">
                                    {subscription.requester?.name || 'N/A'}
                                    {subscription.requester?.email && (
                                        <span className="text-secondary-400 ml-1">({subscription.requester.email})</span>
                                    )}
                                </dd>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <dd className="mt-1">
                                    <Badge variant={statusVariants[subscription.status]} size="sm">
                ...
                <div className="space-y-6">
                    ...
                    {/* Club Info */}
                    <Card>
                        <h3 className="font-semibold text-secondary-900 dark:text-white mb-3">Club Info</h3>
                        <dl className="space-y-2 text-sm">
                            <div className="flex justify-between">
                                <dt className="text-secondary-500">Name</dt>
                                <dd className="text-secondary-900 dark:text-white font-medium">{subscription.club?.name}</dd>
                            </div>
                            {subscription.club?.owner && (
                                <div className="flex justify-between">
                                    <dt className="text-secondary-500">Owner</dt>
                                    <dd className="text-secondary-900 dark:text-white">{subscription.club.owner.name}</dd>
                                </div>
                            )}
                        </dl>
                    </Card>
