...
export default function GarageEdit({ id, bike }: GarageEditProps) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT',
        make: bike?.make || '',
        model: bike?.model || '',
        year: bike?.year || currentYear,
        cc: bike?.cc?.toString() || '',
        category: bike?.category || '',
        modification_level: bike?.modification_level || 'stock',
        color: bike?.color || '',
    ...
    if (!bike) {
        return (
            <AuthenticatedLayout header="Edit Bike">
    ...
    return (
        <AuthenticatedLayout header={`Edit ${bike.make} ${bike.model}`}>
            <Head title={`Edit ${bike.make} ${bike.model}`} />
                ...
                <Card padding="lg">
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        <div className="flex items-center justify-end gap-4 pt-4">
                            <Link href={route('garage.show', id)}>
